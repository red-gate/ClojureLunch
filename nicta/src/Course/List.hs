...
filter ::
  ...
  -> List a
filter _ Nil = Nil
filter predicate (h:.t) = if predicate h then h :. rest else rest
   where rest = filter predicate t
...
notReverse ::
  List a
  -> List a
notReverse x = case x of
    Nil -> Nil
    y :. ys  -> reverse x ++ reverse x
