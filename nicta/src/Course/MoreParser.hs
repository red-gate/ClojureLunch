...
noneof ::
  Chars
  -> Parser Char
noneof xs = satisfy (\y -> notElem y xs)
...
hex ::
  Parser Char
hex = do
   digits <- replicateA 4 (satisfy isHexDigit)
   let Full r = readHex digits
   return $ chr r
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma c e p = betweenCharTok c e (sepby p (charTok ',')) 
