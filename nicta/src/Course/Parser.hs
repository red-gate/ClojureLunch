...
list1 ::
  Parser a
  -> Parser (List a)
list1 p = bindParser (\a -> mapParser (a :.) (list p)) p
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy pred = flbindParser character (\c -> if pred c then valueParser c else unexpectedCharParser c) 
...
natural ::
  Parser Int
natural = flbindParser (list1 digit) (\y -> case read y of 
                                          Empty -> error "No digits read"
...
sequenceParser (p:.ps) = flbindParser p (\x -> mapParser (x:.) (sequenceParser ps))
...
surnameParser = flbindParser  upper (\c -> 
   flbindParser (thisMany 5 lower) (\ls ->      
      flbindParser (list lower) (\rest -> 
        valueParser ((c :. ls) ++ rest)
      )
      ))
